 ...
 * @author Win 10
 */
@WebServlet(name = "AdminUpdateItemServlet", urlPatterns = {"/AdminUpdateItemServlet"})
@MultipartConfig(fileSizeThreshold = 0, maxFileSize = 20480, maxRequestSize = 20480)
public class AdminUpdateItemServlet extends HttpServlet {
    private static final String UPDATE_PAGE="/updateItem.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(checkMultiPart){
                ...
                if(isError){
                    session.setAttribute("error", itemError);
                }else{
                    ...
                    if(categoryId.isEmpty() || statusId.isEmpty() || userId==null || itemId==null){
                        ...
                        if (image.trim().isEmpty()) {
                            if (itemDAO.updateItemExceptImage(itemName, Integer.parseInt(stringPrice), categoryId, 
                                    Integer.parseInt(stringQuantity), convertStringToSQLDate(stringExpirationDate), statusId, userId, itemId)) {
                                url=ADMIN_TABLE_CONTROLLER;
                            }
                        } else {
                            if (itemDAO.updateItem(itemName, image, Integer.parseInt(stringPrice), categoryId, 
                                    Integer.parseInt(stringQuantity), convertStringToSQLDate(stringExpirationDate), statusId, userId, itemId)) {
                                url=ADMIN_TABLE_CONTROLLER;
                            }
                        }
